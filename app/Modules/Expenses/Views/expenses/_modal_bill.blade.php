    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">@lang('fi.bill_this_expense')</h4>
                ...
                <form>
                    ...
                    @if ($invoices)
                        <div class="form-group">
                            <label class="col-form-label">* @lang('fi.label_invoice'):</label>
                            {!! Form::select('invoice_id', $invoices, null, ['id' => 'invoice_id', 'class' => 'form-control']) !!}
                        </div>

                        <div class="form-group">
                            <label class="col-form-label">{!! Form::radio('add_line_item', 1, true, ['class' => 'add-line-item']) !!} @lang('fi.add_line_item_to_invoice')</label><br>
                            <label class="col-form-label">{!! Form::radio('add_line_item', 0, false, ['class' => 'add-line-item']) !!} @lang('fi.do_not_add_line_item_to_invoice')</label>
                        </div>

                        <div id="line-item-options">
                            <div class="form-group">
                                <label class="col-form-label">* @lang('fi.label_item_name'):</label>
                                {!! Form::text('item_name', $expense->category->name, ['id' => 'item_name', 'class' => 'form-control']) !!}
                            </div>

                            <div class="form-group">
                                <label class="col-form-label">@lang('fi.label_item_description'):</label>
                                {!! Form::textarea('item_description', $expense->description, ['id' => 'item_description', 'rows' => '3', 'class' => 'form-control']) !!}
                            </div>
                        </div>
                    @else
                        <p>@lang('fi.no_open_invoices')</p>
                    @endif
            ...
            <div class="modal-footer">
                ...
                @if ($invoices)
                    <button type="button" id="btn-create-expense-bill-confirm" class="btn btn-primary">@lang('fi.submit')</button>
                @endif
            </div>
        </div>
    </div>
