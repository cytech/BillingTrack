@extends('documentation.master')

@section('content')

    <div class="col-lg-9 documentation">

        <h2>Utilities</h2>

        <hr>

        <p style="margin-bottom: 0; line-height: 100%"><a name="__DdeLink__0_310968197"></a>
            <b>Employees</b></p>
        <p style="margin-bottom: 0; font-weight: normal; line-height: 100%">
            Allows for creation of Employees that may be assigned to workorders.
            The list may be sorted by the available header fields. To edit an
            Employee, select the Employee # in the list.</p>
        <p style="margin-left: 0.49in; margin-bottom: 0; line-height: 100%">
            <b>New</b></p>
        <p style="margin-left: 0.49in; margin-bottom: 0; font-weight: normal; line-height: 100%">
            Create a new employee.</p>
        <p style="margin-left: 0.49in; margin-bottom: 0; line-height: 100%">
            <br/>

        </p>
        <p style="margin-left: 0.49in; margin-bottom: 0; line-height: 100%">
            <br/>

        </p>
        <p style="margin-bottom: 0; line-height: 100%"><b>Products</b></p>
        <p style="margin-bottom: 0; font-weight: normal; line-height: 100%">
            Allows for creation of Products (inventory or standard job items)
            that may be assigned to documents. The list may be sorted by the
            available header fields. To edit a Product, select the Product # in
            the list.</p>
        <p style="margin-left: 0.49in; margin-bottom: 0; line-height: 100%">
            <b>New</b></p>
        <p style="margin-left: 0.49in; margin-bottom: 0; font-weight: normal; line-height: 100%">
            Create a new Product.</p>
        <p style="margin-bottom: 0; line-height: 100%"><br/>

        </p>
        <p style="margin-bottom: 0; line-height: 100%"><br/>

        </p>
        <p style="margin-bottom: 0; line-height: 100%"><br/>

        </p>
        <p style="margin-bottom: 0; line-height: 100%"><br/>

        </p>

        <p style="margin-left: 0.0in; margin-bottom: 0; line-height: 100%">
            <b>Batch Print</b></p>
        <p style="margin-left: 0.49in; margin-bottom: 0; font-weight: normal; line-height: 100%">
            Allows PDF creation of multiple workorders by selecting a start and
            end job date. Only workorders that are approved, not invoiced and
            meet the job date criteria are selected.</p>

        <p style="margin-left: 0.0in; margin-bottom: 0; line-height: 100%">
            <b>Item Lookups</b></p>
        <p style="margin-left: 0.49in; margin-bottom: 0; font-weight: normal; line-height: 100%">
            Item Lookups.</p>

        <p style="margin-left: 0.0in; margin-bottom: 0; line-height: 100%">
            <b>Mail Log</b></p>
        <p style="margin-left: 0.49in; margin-bottom: 0; font-weight: normal; line-height: 100%">
            Mail Log.</p>

        <p style="margin-left: 0.0in; margin-bottom: 0; line-height: 100%">
            <b>Manage Trash</b></p>
        <p style="margin-left: 0.49in; margin-bottom: 0; font-weight: normal; line-height: 100%">
            Manage Trash.</p>

    </div>

@stop