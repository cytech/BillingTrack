<script type="text/javascript">
    ...
    $(function () {
        ...
        var itemLookups = new Bloodhound({
            ...
            remote: '{{ route('itemLookupswo.ajax.itemLookup') }}' + '?query=%QUERY'
        });
        ...
        function typeaheadTrigger() {
            $('.item-lookup').on('typeahead:selected typeahead:autocompleted', function (obj, item, name) {
                ...
                row.find('input[name="resource_table"]').val(item.resource_table);
                row.find('input[name="resource_id"]').val(item.resource_id);
